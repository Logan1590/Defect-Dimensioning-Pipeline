...
from ML_Segmentation_v25 import run_detection_pipeline  # Your detector
...
load_reference_keypoints(json_path, image_name)
    Loads 2D keypoints from a JSON file for a given image and associates them with 3D CAD points using ID matching. Returns image and object point lists.

filter_matches_by_best_proximity(good_matches, kp1, kp2, max_pixel_dist=None)
    Filters SIFT keypoint matches by selecting only reciprocal closest matches within an optional pixel distance threshold. Improves robustness of 2D–3D correspondences.
...
def estimate_pose(real_img_path, reference_img_path, json_path, K, dist_coeffs=None, max_keypoints=50, compute_ref_pose=False, visualize=False):
    ...
    if len(matched_3D) >= 4:
        ...
        if success:
            print("Pose estimation with solvePnPRansac successful.")
        else:
